...
import {readJsonWebToken, readChannelName} from '../../reducer';
...
export const connectToChannel = (socket, dispatch) => {
  channel = socket.channel(`secrets:${readChannelName()}`, {
    guardian_token: readJsonWebToken(),
  });
  ...
  channel.on('sync:end', data => {
    loadSecrets().then(secretsFromDb => {
      data.secrets.forEach(secret => {

        if (!secretsFromDb.find(s => s.uuid == secret.uuid)) {
...
const loadAndSyncSecrets = channel => {
  Promise.all([loadSecrets(), loadEvents()]).then(values => {
    // Delete id field
    ...
    const events = values[1].reduce((acc, event) => {
      ...
      return acc.map(e => e.uuid).includes(event.uuid)
        ? acc
        : acc.concat(event);
    }, []);
